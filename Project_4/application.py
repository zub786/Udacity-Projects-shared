    ...
    try:
        ...
        if request.method == 'GET':
            ...
            if user:
                ...
                return render_template('loginAgain.html', Message = "User Not Found!")
    except:
...
def signup():
    try:
        ...
        if request.method == 'GET':
            ...
            user = User()
            isExist = session.query(User).filter_by(email = request.form.get('username')).all()
            if(isExist.__len__() == 0):
                ...
                return render_template('signupAgain.html', Message = "Sorry! user already exist against this email address")
    except:
        return redirect('/error')
...
def gconnect():
    try:
        ...
        output += ' " style = "width: 150px; height: 150px;border-radius: 100px;-webkit-border-radius: 150px;-moz-border-radius: 150px;"> '
...
def createUser(login_session):
    try:
        user = User(username = login_session['username'], email = login_session['email'], picture = login_session['picture'])
        session = DBSession()
        session.add(user)
        session.commit()
...
def showCategories():
    try:
        if login_session.get('logged_in'):
            ...
            session = DBSession()
            categories = session.query(Category).all()
            itemsids = session.query(func.max(Item.id)).group_by(Item.category_id).all()
            itemsidsString = str(itemsids).replace('(', '').replace(')', '')
            items = session.query(Item).filter(Item.id.in_(str(itemsidsString).strip('[]'))).all()  
            ...
            itemsidsString = str(itemsids).replace('(', '').replace(')', '')
            items = session.query(Item).filter(Item.id.in_(str(itemsidsString).strip('[]'))).all()  
            return render_template('categoriesPublic.html', categories = categories, items = items)
    except:
        return redirect('/error')
